...
(function(Scratch) {
    ...
    class SVG3D {
                ...
                blocks: [
                    {
                        ...
                        text: 'Get render [TYPE] with width [WIDTH] and height [HEIGHT]',
                        arguments: {
                    ...
                    },
                    {
                        ...
                        arguments: {
                            MIN: { type: Scratch.ArgumentType.NUMBER, defaultValue: '-500' },
        ...
        getRender({ TYPE, WIDTH, HEIGHT }) {
            if (this.illo.isCanvas) {
                if (TYPE === 'svg') {
                    this.illo = new Zdog.Illustration({
                        element: this.renders.svg,
                    });
                }
            } else {
                this.illo = new Zdog.Illustration({
            ...
            if (this.illo.isCanvas) {
                return this.renders.canvas.toDataURL(TYPE);
            } else {
                const serializer = new XMLSerializer();
                const svgString = serializer.serializeToString(this.renders.svg);
    ...
    Scratch.extensions.register(new SVG3D());
})(Scratch);
