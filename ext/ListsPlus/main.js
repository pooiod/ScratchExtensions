...
(function(Scratch) {
    ...
    class ListPlus {
        ...
        isList(list) {
            try {
                return Array.isArray(JSON.parse(list));
            } catch (e) {
                return false;
            }
        }

        createListFromText({ TEXT }) {
            return '["' + TEXT.replace(/, /g, '", "') + '"]';
        }

        getItem({ INDEX, LIST }) {
            if (!this.isList(LIST)) return;

            const index = Math.max(1, Math.floor(INDEX));
